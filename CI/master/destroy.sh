...
central_vm_id=$(python utils.py --get id --vm-name "Central VM master CI" --auth-token $okeanos_token)
...
echo "$(python manage_lambda_instance.py --action destroy --service-vm-name "Service VM master CI" --auth-token $okeanos_token)"

# Get the id of the Service VM.
service_vm_id=$(python utils.py --get id --vm-name "Service VM master CI" --auth-token $okeanos_token)
