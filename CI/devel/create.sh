...
echo "$(python manage_lambda_instance.py --action create --service-vm-name "Service VM devel CI" --auth-token $okeanos_token)"
...
echo "$(python manage_application.py --action upload --service-vm-name "Service VM devel CI" --auth-token $okeanos_token --application stream-1.0-jar-with-dependencies.jar)"
echo "$(python manage_application.py --action deploy --service-vm-name "Service VM devel CI" --auth-token $okeanos_token)"
echo "$(python manage_application.py --action start --service-vm-name "Service VM devel CI" --auth-token $okeanos_token)"
